...
# =========================================================
# 22. OPTIONAL: Prepare Data for AIC/BIC Model Selection
# =========================================================
...
# =========================================================
# 25. Backward Elimination (AIC/BIC)
# =========================================================

def backward_elimination(Xmat, yvec, criterion="AIC"):        # backward elimination function
    ...
    while len(selected) > 1:                                  # loop until only one predictor left
        ...
        for cand in selected:                                 # test removing each predictor
            cols = [0] + [c for c in selected if c != cand]   # intercept + all except candidate
            model = fit_ols(Xmat[:, cols], yvec)              # fit model
...
# =========================================================
# 26. Stepwise Selection (AIC/BIC)
# =========================================================

def stepwise_selection(Xmat, yvec, criterion="AIC"):          # stepwise selection function
    remaining = list(range(1, Xmat.shape[1]))                 # predictors not yet selected
    selected = []                                             # selected predictors
    current_score = np.inf                                    # best score
    changed = True                                            # flag for loop

    while changed:                                            # loop until no change
        changed = False                                       # reset flag

        # Forward step
        forward_scores = []                                   # stores forward scores
        for cand in remaining:                                # test adding each predictor
            cols = [0] + selected + [cand]                    # intercept + selected + candidate
            model = fit_ols(Xmat[:, cols], yvec)              # fit model
            score = model.aic if criterion == "AIC" else model.bic
            forward_scores.append((score, cand))              # store score + candidate

        forward_scores.sort()                                 # sort by best score
        best_forward_score, best_forward_cand = forward_scores[0]

        if best_forward_score < current_score:                # if improvement
            selected.append(best_forward_cand)                # add predictor
            remaining.remove(best_forward_cand)               # remove from remaining
            current_score = best_forward_score                # update score
            changed = True                                    # mark change

        # Backward step
        if len(selected) > 1:                                 # only if >1 predictors selected
            backward_scores = []                              # stores backward scores
            for cand in selected:                             # test removing each predictor
                cols = [0] + [c for c in selected if c != cand]
                model = fit_ols(Xmat[:, cols], yvec)
                score = model.aic if criterion == "AIC" else model.bic
                backward_scores.append((score, cand))

            backward_scores.sort()                            # sort by best score
            best_backward_score, worst_cand = backward_scores[0]

            if best_backward_score < current_score:           # if improvement
                selected.remove(worst_cand)                   # remove predictor
                current_score = best_backward_score           # update score
                changed = True                                # mark change

    return selected, current_score                            # return selected predictors + score

# =========================================================
# 27. Run All Model Selection Procedures
# =========================================================

fwd_aic_vars, fwd_aic_score = forward_selection(X_transformed, y_array, "AIC")  # forward AIC
fwd_bic_vars, fwd_bic_score = forward_selection(X_transformed, y_array, "BIC")  # forward BIC

bwd_aic_vars, bwd_aic_score = backward_elimination(X_transformed, y_array, "AIC")  # backward AIC
bwd_bic_vars, bwd_bic_score = backward_elimination(X_transformed, y_array, "BIC")  # backward BIC

step_aic_vars, step_aic_score = stepwise_selection(X_transformed, y_array, "AIC")  # stepwise AIC
step_bic_vars, step_bic_score = stepwise_selection(X_transformed, y_array, "BIC")  # stepwise BIC

print("\n===== MODEL SELECTION RESULTS =====")
print(f"Forward AIC vars: {fwd_aic_vars}, AIC: {fwd_aic_score}")                   # forward AIC results
print(f"Forward BIC vars: {fwd_bic_vars}, BIC: {fwd_bic_score}")                   # forward BIC results
print(f"Backward AIC vars: {bwd_aic_vars}, AIC: {bwd_aic_score}")                 # backward AIC results
print(f"Backward BIC vars: {bwd_bic_vars}, BIC: {bwd_bic_score}")                 # backward BIC results
print(f"Stepwise AIC vars: {step_aic_vars}, AIC: {step_aic_score}")               # stepwise AIC results
print(f"Stepwise BIC vars: {step_bic_vars}, BIC: {step_bic_score}")               # stepwise BIC results

# =========================================================
# 28. Fit Final Model (Example: Stepwise AIC)
# =========================================================

final_cols = [0] + step_aic_vars                               # intercept + selected predictors
final_model = fit_ols(X_transformed[:, final_cols], y_array)   # fits final OLS model

print("\n===== FINAL MODEL SUMMARY (Stepwise AIC) =====")
print(final_model.summary())                                   # prints full OLS summary
